...
// FIX: svg2pdf should be imported as a **default** export in ESM
...
type PlinthSystem = "beton" | "ceownik"; // 2450 / 2500

interface SectionCfg {
  name: SectionName;
  enabled: boolean;
  length: number; // in current unit
  system: PlinthSystem; // per bok
}
...
export default function FencePlanner() {
  ...
  const [gate, setGate] = useState({ enabled: true, section: "front" as SectionName, width: 4, height: 1.6, offset: 3 });
  const [wicket, setWicket] = useState({ enabled: true, section: "front" as SectionName, width: 1, height: 1.6, offset: 8 });

  // Dev: szybki wynik testów (brak frameworka testowego w tym pliku)
  ...
  const result = useMemo(() => {
    ...
    const calcOne = (lengthMM: number, withGate: boolean, withWicket: boolean) => {
      ...
      const postsLinear = n + 1; // słupki na panelach (bez rogów)
    ...
    // policz sekcje
    const active = mm.sections.filter((s) => s.enabled && s.length > 0);
    ...
    inOrder.forEach((s) => {
      ...
      const r = calcOne(s.length, hasGate, hasWicket);
      perSection.push({ name: s.name, system: s.system, ...r });
    });

    // narożniki: między sąsiadującymi WŁĄCZONYMI bokami (w kolejności front→right→back→left→front)
    let corners = 0;
    for (let i = 0; i < order.length; i++) {
      const a = mm.sections.find((s) => s.name === order[i])!;
      const b = mm.sections.find((s) => s.name === order[(i + 1) % order.length])!;
      if (a.enabled && a.length > 0 && b.enabled && b.length > 0) corners++;
    }

    // Słupki całkowite: zsumuj liniowe, uwzględniając, że sąsiadujące sekcje dzielą słupek narożny (liczony w corners)
    const postsLinearSum = perSection.reduce((acc, s) => acc + s.postsLinear, 0);
    const totalPosts = postsLinearSum + corners; // dodaj jeden słupek narożny na każdy narożnik

    // Obejmy: na każdy słupek tyle co przetłoczeń; dla rogowych słupków użyjemy typu „rogowa”
    const clampsPerPost = corr;
    const totalClamps = clampsPerPost * totalPosts;
    const cornerClamps = clampsPerPost * corners; // rogowe

    // Łączniki narożne podmurówki
    const cornerBeton = inOrder.length
      ? countCornersBySystem(mm.sections as any, "beton")
      : 0;
    const cornerCeownik = inOrder.length
      ? countCornersBySystem(mm.sections as any, "ceownik")
      : 0;
  ...
  function countCornersBySystem(sectionsArr: SectionCfg[], sys: PlinthSystem) {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      if (a.enabled && a.length > 0 && b.enabled && b.length > 0) {
        // jeżeli którykolwiek bok z pary jest danego systemu – zalicz narożnik do tego systemu
  ...
  const buildBOM = () => {
    ...
    // sumy globalne
    it.push({ name: "Panel ogrodzeniowy", qty: result.nPanels, details: `typ ${inputs.panelType}, szer. ${format(mm.panelW, u, 2)}, wys. ${format(mm.panelH, u, 2)}` });
    it.push({ name: "Słupek", qty: result.posts, details: `przekrój ${format(mm.postWidth, u, 3)} × ${format(mm.postWidth, u, 3)}` });

    // akcesoria
    if (result.clampsPerPost > 0) {
      it.push({ name: "Obejma montażowa (prosta)", qty: result.totalClamps - result.cornerClamps, details: `${result.clampsPerPost} szt/ słupek liniowy` });
      it.push({ name: "Obejma montażowa (rogowa)", qty: result.cornerClamps, details: `${result.clampsPerPost} szt/ słupek narożny` });
    }

    // podmurówka – plinth per panel
    it.push({ name: "Podmurówka prefab.", qty: result.plinths, details: `pod panelem: ${format(mm.panelW, u, 2)} × wys. ${format(mm.footing, u, 2)}` });

    // łączniki narożne
    if (result.cornerBeton > 0) it.push({ name: "Łącznik betonowy narożny", qty: result.cornerBeton, details: `dla płyt 2450 mm` });
    if (result.cornerCeownik > 0) it.push({ name: "Ceownik narożny (do weryfikacji)", qty: result.cornerCeownik, details: `dla płyt 2500 mm` });

    // TODO: dodać bramę/furtkę gdy będą rozbite per sekcja (tu liczymy pozycje osobno w UI)
    if (gate.enabled) it.push({ name: "Brama", qty: 1, details: `bok: ${gate.section}, szer. ${format(mm.gate.width, u, 2)}, wys. ${format(mm.gate.height, u, 2)}` });
    if (wicket.enabled) it.push({ name: "Furtka", qty: 1, details: `bok: ${wicket.section}, szer. ${format(mm.wicket.width, u, 2)}, wys. ${format(mm.wicket.height, u, 2)}` });
  ...
  const downloadPDF = () => {
    ...
    const doc = new jsPDF({ orientation: bbox.width > bbox.height ? "l" : "p", unit: "mm", format: "a4" });
    ...
    (doc as any).autoTable({ startY: pageH - 65, head: [["Element", "Ilość", "Szczegóły"]], body: rows, styles: { fontSize: 9 } });
  ...
  const svg = useMemo(() => {
    ...
    inOrder.forEach((s, idx) => {
      ...
      // etykieta boku
      elems.push(<text key={`label-${s.name}`} x={cursor + w / 2} y={lineY + 16} textAnchor="middle" fontSize={12}>{label}</text>);
      ...
      elems.push(<text key={`sys-${s.name}`} x={cursor + w / 2} y={lineY - h - 6} textAnchor="middle" fontSize={10}>{s.system === 'beton' ? 'Łącznik betonowy (2450)' : 'Ceownik (2500)'}</text>);

      // narożnik po prawej (jeśli kolejny odcinek też istnieje lub domykamy do pierwszego)
      const next = inOrder[(idx + 1) % inOrder.length];
      if (next) {
        const isCorner = true; // wszystkie sąsiadujące w inOrder traktujemy jako narożnik
        if (isCorner) {
          const x = cursor + w;
  ...
  // Presety odcinków – szybkie przyciski
  ...
  // --- Minimalne testy runtime (brak frameworka testowego) ---
  const runSelfTests = () => {
    const results: string[] = [];
    // Test 1: svg2pdf import shape
    results.push(typeof svg2pdf === "function" ? "PASS: svg2pdf is a function (default import)" : `FAIL: svg2pdf typeof=${typeof svg2pdf}`);
    // Test 2: BOM sums are non-negative
    const bom = buildBOM();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <motion.h1 className="text-2xl font-semibold flex items-center gap-2"><Layout className="w-6 h-6"/> Planer ogrodzeń – kalkulator i rysunek 2D</motion.h1>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {/* GLOBAL */}
        <Card className="xl:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Calculator className="w-5 h-5"/> Parametry ogólne</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="col-span-2 border-t pt-2"><Label className="flex items-center gap-2"><Ruler className="w-4 h-4"/> Przerwy (luz montażowy)</Label></div>
            ...
            <div className="col-span-2 border-t pt-2"><Label>Akcesoria / obejmy</Label></div>
            <NumberField label="Liczba przetłoczeń (= obejm na słupek)" value={(inputs.corrugations as number) ?? ("") as any} onChange={setInput("corrugations")} unit={"szt"}/>
            <p className="col-span-2 text-xs text-muted-foreground">Dla 2D przyjmujemy liczbę obejm jak w 3D o tej samej wysokości. Tymczasowo: 830→2; 2230/2430→4 (pozostałe do uzupełnienia).</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={() => window.location.reload()} variant="outline" className="gap-2"><RefreshCw className="w-4 h-4"/> Reset</Button>
            </div>
            <div className="flex gap-2 pt-2">
              <Button onClick={downloadSVG} className="gap-2"><Download className="w-4 h-4"/> SVG</Button>
              <Button onClick={downloadPDF} variant="secondary" className="gap-2"><Download className="w-4 h-4"/> PDF</Button>
              <Button onClick={downloadCSV} variant="secondary" className="gap-2"><Download className="w-4 h-4"/> CSV</Button>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Map className="w-5 h-5"/> Boki działki</CardTitle>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {sections.map((s, i) => (
              <div key={s.name} className={`border rounded-md p-3 space-y-2 ${s.enabled ? '' : 'opacity-60'}`}>
                ...
                <NumberField label="Długość" value={s.length as any} onChange={(v: any) => setSection(i, { length: Number(v?.target?.value ?? v) })} unit={unit}/>
                <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Bug className="w-5 h-5"/> Testy (runtime)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Te szybkie testy sprawdzają import svg2pdf i podstawy BOM bez negatywnych ilości.</p>
        </CardContent>
      </Card>
...
function NumberField({ label, value, onChange, unit }: { label: string; value: number; onChange: (v: any) => void; unit: string }) {
  return (
